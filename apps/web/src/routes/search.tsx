...
function SearchComponent() {
  ...
  const handleSaveSearch = useCallback(() => {
    ...
    if (!isLoggedIn) {
      ...
      alert(
        "Please login to save your search. You'll receive email notifications when new matches are found."
      );
    ...
    try {
      ...
      setTimeout(() => {
        setIsSaving(false);
        alert(
          "Search saved! You'll receive email notifications when new matching listings are added."
        );
      }, 1000);
    } catch (error) {
      setIsSaving(false);
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        ...
        <div className="space-y-1">
          <p className="text-muted-foreground">
            Found {filteredListings.length} available listing
          ...
          {(search.adults || search.children) && (
            <p className="text-sm text-muted-foreground">
              ✓ Prices updated for {Number(search.adults || 2) + Number(search.children || 0)} guest
              {Number(search.adults || 2) + Number(search.children || 0) !== 1 ? 's' : ''}
      ...
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              updateSearch({ flashDeals: search.flashDeals === 'true' ? undefined : 'true' })
            }
            className={
              search.flashDeals === 'true'
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              updateSearch({ verifiedOnly: search.verifiedOnly === 'true' ? undefined : 'true' })
            }
          >
            Verified Only
          </Button>
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              updateSearch({ groupEligible: search.groupEligible === 'true' ? undefined : 'true' })
            }
          >
            Group Bookings
          </Button>
      ...
      {showFilters && (
        <div className="mb-6 rounded-lg border bg-muted/30 p-6">
          ...
          <div className="mb-6 pb-6 border-b">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <DateRangePicker
                ...
                onCheckInChange={(date) => updateSearch({ checkIn: date?.toISOString() })}
                onCheckOutChange={(date) => updateSearch({ checkOut: date?.toISOString() })}
              />
              ...
              <GuestSelector
                ...
                onChildrenChange={(value) => updateSearch({ children: value.toString() })}
          ...
          <div className="mb-6 pb-6 border-b">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <PriceRangeSlider
                min={0}
                max={50000}
                value={[Number(search.minPrice || 0), Number(search.maxPrice || 50000)]}
          ...
          <div className="mb-6">
            ...
            <AmenitiesFilter
              selectedAmenities={search.amenities ? search.amenities.split(',') : []}
              onChange={(amenities) =>
                updateSearch({
      ...
      {error && (
        <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-8 text-center">
          <h3 className="mb-2 text-lg font-semibold text-destructive">Failed to load listings</h3>
          <p className="text-sm text-muted-foreground">
      ...
      {!isLoading && !error && viewMode === 'map' && (
        <div className="rounded-lg border bg-muted/30 p-8 text-center">
          ...
          <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-4 auto-rows-fr">
            {filteredListings.map((listing) => (
              <ListingCard
                key={listing.id}
                listing={listing}
      ...
      {filteredListings.length === 0 && (
        <div className="py-12 text-center">
          ...
          <Button onClick={() => navigate({ to: '/search', search: {} })}>Clear All Filters</Button>
        </div>
      )}
