...
const BusinessInfoSchema = z
  .object({
    ...
    businessDescription: z.string().optional().openapi({
      example:
        'Leading tour operator specializing in cultural and heritage tours across Bangladesh',
...
const SellerSchema = z
  .object({
    ...
    verificationStatus: z
      .enum(['pending', 'in-review', 'approved', 'rejected', 'incomplete'])
      .openapi({
...
const DocumentSchema = z
  .object({
    ...
    fileUrl: z.string().url().openapi({
      example:
        'http://127.0.0.1:54321/storage/v1/object/sign/seller-documents/sel_abc/doc.pdf?token=xyz&exp=1738746000',
    ...
    storageKey: z.string().openapi({
      description:
        'Supabase Storage key for file management (e.g., sellerId/documentType_timestamp.ext)',
    }),
...
export const registerSellerRoute = createRoute({
  ...
  description:
    'Creates an initial seller record for a user. This is the first step in the seller onboarding process. Returns an error if a seller account already exists for the given user ID.',
  request: {
    body: {
      content: {
        'application/json': {
          schema: z.object({
            userId: z.string().min(1).openapi({
              example: 'user_1738367890_xyz456',
              description:
                'User ID from the authentication system. Must be a valid authenticated user.',
            }),
          }),
        },
      },
  ...
  responses: {
    201: {
      description:
        'Seller registered successfully. A new seller account has been created with pending status.',
      content: {
        'application/json': {
          schema: z.object({
            sellerId: z.string().openapi({
    ...
    400: {
      description:
        'Bad request. Either user ID is missing or a seller account already exists for this user.',
      content: {
        'application/json': {
          schema: ErrorSchema,
        },
...
export const getSellerByUserRoute = createRoute({
  ...
  description:
    'Retrieves seller information associated with a given user ID. Returns null in the seller field if no seller account exists for this user.',
  request: {
    params: z.object({
      userId: z
        .string()
  ...
  responses: {
    200: {
      description:
        'Request successful. Returns seller data if exists, or null if no seller account found.',
      content: {
        'application/json': {
          schema: z.object({
...
export const completeOnboardingRoute = createRoute({
  ...
  description:
    'Submits complete seller information including business details and bank account. This is the final step in the seller onboarding process. Updates the seller verification status to pending and creates a bank account record and timeline event.',
  request: {
    body: {
      content: {
        'application/json': {
  ...
  responses: {
    200: {
      description:
        'Onboarding completed successfully. Seller information has been saved and verification process has started.',
      content: {
        'application/json': {
          schema: z.object({
            sellerId: z.string().openapi({
...
export const getVerificationStatusRoute = createRoute({
  ...
  description:
    "Retrieves complete verification status for a seller including seller information, uploaded documents, verification timeline events, and bank account details. This endpoint provides a comprehensive view of the seller's verification progress.",
  request: {
    params: z.object({
      sellerId: z
        .string()
